...
```mermaid
 graph TD
    A[发布者 Publisher] -->|发布消息| B[Redis服务器]
...
![img](http://img.lindaifeng.vip/typora-picgo-tuchaung/db-redis-sub-8.png)

在这个流程中，发布者发送消息到指定的频道(channel)，Redis 服务器负责将消息分发给所有订阅了该频道的订阅者
...
如果有某个/某些模式(匹配规则)和这个频道匹配的话，那么所有订阅这个/这些频道的客户端也同样会收到信息。

如图：其中 channel.* 模式匹配了两个频道，每个频道上各自都有订阅者。

当消息发布到任意频道上，订阅了该频道的订阅者和匹配上该频道的订阅者就都能收到消息

publisher1发布消息到频道channel.A，则subsciber 1和subsciber 3，subsciber 4 都能收到消息

 ![iShot_2025-08-28_16.33.36](http://img.lindaifeng.vip/typora-picgo-tuchaung/iShot_2025-08-28_16.33.36.png)



## Spring Boot 集成 Redis 发布订阅
